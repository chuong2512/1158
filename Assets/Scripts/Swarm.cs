...
public class Swarm : MonoBehaviour
{
	...
	private void Start()
	{
		...
		for (int i = 0; i < this.birdsCount; i++)
		{
			...
			GameObject gameObject = UnityEngine.Object.Instantiate<GameObject>(this.bird, vector, base.transform.rotation);
	...
	private Vector3 GetNewPos()
	{
		Vector3 result;
		result.x = 0f;
		result.y = Mathf.Sin(this.angle * 0.0174532924f) * this.amplitude;
		result.z = 0f;
		return result;
	}
}
